...
def getLastFiles():
    xlsFilePath = os.path.join(CWD, 'xlsFiles')
    xlsFiles = [f for f in os.listdir(xlsFilePath) if(f.endswith('.xls'))]
    xlsFiles.sort(reverse=True)
    lastFive = xlsFiles[0:4]
    return lastFive
...
def getNextDate(dateStr):
    ...
    return nextDate.strftime("%Y%m%d") + '_' + days[nextDate.weekday()], nextDate.weekday()
